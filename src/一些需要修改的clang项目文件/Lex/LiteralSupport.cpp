  ...
  while (begin != end) {
    ...
    if (begin[0] != '\\') {
      ...
      if (res != conversionOK) {
        ...
        unsigned Msg = diag::warn_bad_character_encoding; // diag::err_bad_character_encoding;
...
bool StringLiteralParser::CopyStringFragment(const Token &Tok,
  ...
  if (Diags) {
	...
	const DiagnosticBuilder &Builder =
		...
		NoErrorOnBadEncoding ? diag::warn_bad_string_encoding : diag::warn_bad_string_encoding);
